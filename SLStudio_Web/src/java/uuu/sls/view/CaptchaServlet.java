...
public class CaptchaServlet extends HttpServlet {
    ...
    private int len = 6, width = 20 * 2 + 12 * len, height = 40;
    ...
    private BufferedImage generateImage(String rand, int width, int height) {
        ...
        //畫出認證文字
        g.setFont(new Font("Arial", Font.ITALIC, 20));//設定字體

        g.setColor(getRandomColor(20, 140));

        //將認證文字畫到image中
        g.drawString(rand, 20, 25);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //1.用亂數產生0-9,A-Z(36)字元共6個的字串
        String rand = "";
        Random r = new Random();
        for (int i = 0; i < len; i++) {
            int n = r.nextInt(36);
            char data = n < 10 ? (char) (n + '0') : (char) (n - 10 + 'A');
            rand = rand + data;
        }
